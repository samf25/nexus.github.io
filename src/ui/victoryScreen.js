...
function drawSymbolTracer(controller, tracer, elapsedSec, rotY, rotX, width, height, globalScale, depth, palette) {
  ...
  const showTrail = age < flyEnd;
  if (showTrail) {
    tracer.trail.unshift(head);
    if (tracer.trail.length > 128) {
      tracer.trail.pop();
    }
  ...
  if (showTrail) {
    ctx.strokeStyle = `hsla(${traceHue.toFixed(1)}, ${palette.traceSat.toFixed(1)}%, ${palette.traceLight.toFixed(1)}%, ${(0.72 * trailAlpha).toFixed(3)})`;
    ctx.lineWidth = 2.2;
    ctx.beginPath();
    let moved = false;
    for (let index = 0; index < tracer.trail.length; index += 1) {
      const projected = projectPoint(rotatePoint(tracer.trail[index], rotY, rotX), width, height, globalScale, depth);
      if (!projected.visible) {
        continue;
      }
      if (!moved) {
